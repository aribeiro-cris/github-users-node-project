...
const fetchUserFromGithub = async (username: string) => {
    try {
        ...
        let data: UserFetchResponse = await response.json() as UserFetchResponse;
...
const fetchLanguagesFromUser = async (reposUrl: string): Promise<string[]> => {
    try {
       ...
       for (const repository of repositories) {
        const repoLanguages = await fetchLanguagesFromRepo(repository.languages_url);
...
const fetchLanguagesFromRepo = async(languagesUrl: string) => {
    try {
